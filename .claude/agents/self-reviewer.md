...
**`Read`, `Grep` and `Glob` are for verification, not for expansion.** Use them to confirm a claim you are about to make — that a quoted rule really says that, that a referenced path exists, that a ✅ status corresponds to something actually present. Do **not** use them to widen the corpus beyond what the caller supplied, to pull in files you were not given, or to substitute your own idea of what should be reviewed. A finding about a file outside the supplied package is out of scope unless it is the *other half* of a contradiction inside it.
