...
8. **Build chain** — dispatch `risk-assessor`, then `code-reviewer`, then parallel reviewers per the step manifest. Run `./scripts/framework/run_tests_inner_loop.sh` after any code change.
   ...
   - **Pre-coder gate (mechanical — blocks coder dispatch).** `scripts/framework/check_pre_coder_gate.sh` is not yet implemented (pending v0.5.0 triage agents, #558). Until it exists: the pipeline discipline rule above is the enforcement mechanism. Do **not** dispatch coder until the appropriate pipeline agents have run.
