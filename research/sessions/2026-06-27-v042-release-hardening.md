...
### Pre-cut governance audit — the fail-open class, sampled at scale (O4)
- **2026-06-27 sequential full-codebase governance audit** (ahead of the v0.5.0 cut, run by Opus): 26 adversarially-verified bugs, **21 fail-open or governance-bypass**, across the validators, the cron loop, the panel, and the red-team checkpoint (filed #703, #910–#925; indexed on #814). Three were cut-blockers — a tautological worker/overseer identity guard (#703), a `--risk` override that skips the panel below the deterministic floor (#910), and a red-team checkpoint that exits 0 with zero reviewers (#911). The meta-finding: nearly every bug violated a principle *already in this corpus*, so the gap is **enforcement, not knowledge** — the case for turning the top principles into lint/CI assertions. Reinforced `unenforceable-rules-need-verification-mechanisms.md` (third instance), `gate-on-computed-signal-not-self-reported-verdict.md`, and OBSERVATIONS O4 in place rather than adding new files (the principles already exist).
...
Almost every incident this period is the *oversight machinery* failing, not the application — and the characteristic catch was the **last-line / pre-release pass finding fail-opens the inner-loop reviewers had approved past** (#814, three at once; the 06-27 pre-cut audit then found 21 more of the same class, confirming it is dense rather than incidental). The period also surfaced a distinct hazard class: the standing autonomous loop accumulates state *between* cycles (stacked branches, committed audit logs, a reset ledger) that a human-in-the-loop process never does, and several fixes are the same shape — *re-derive against durable shared state, never trust the loop's local/ephemeral view* (the git analogue of O1's "agents can't self-certify").
