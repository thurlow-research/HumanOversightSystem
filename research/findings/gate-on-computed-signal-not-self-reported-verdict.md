...
**Corroboration (2026-06-27):** the v0.4.2 pass's three fail-opens were not a fluke but a sample of a population. A sequential full-codebase governance audit ahead of the v0.5.0 cut found **26 verified bugs, 21 of the same fail-open / governance-bypass class**, spread across the validators, the cron loop, the panel, and the red-team checkpoint (#703, #910–#925). The release-boundary catch rate is high because the class is *dense*, not rare — which is exactly why the last-line gate (O8) earns its keep and why these defects need mechanical assertions, not just findings (see `unenforceable-rules-need-verification-mechanisms.md`, third instance).
