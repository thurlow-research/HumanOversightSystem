...
This PR is the **complete** framework upgrade for *this repository*. If your Worker, Human, and Overseer roles share this same GitHub repo (a single-repo deployment), this one PR covers the whole upgrade — you do **not** need a separate PR per role. Only deployments that split the roles across separate repos open one PR per repo (#950)."
