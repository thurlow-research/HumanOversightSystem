...
TRIAGE RULE (for new issues): **priority overrides theme** — a `priority:critical` bug always routes to the active release track (v0.6.1, the open patch line for shipped v0.6.0), never deferred to a later thematic milestone on theme-match grounds alone; an isolated/minor `field-report` fix is biased toward v0.6.1 the same way (not automatic — a field report that's actually a larger design gap still routes by theme). Otherwise: v0.5.1 is drained — closed to new triage, do not select it; v0.6.0 is shipped/closed — bug/patch fixes to its code (node+astro packs, JS validator/gate parity — epic #1029) route to v0.6.1; v0.7.0=quality/non-blocking (new capabilities, measurement) plus worker/overseer finalization plus other governance/security gaps in the active line; v0.7.4=testability (coverage/mutation scope, test-exemption accounting, shell→Python migration of decision logic, CI gates enforcing them); v0.8.0=agility. See docs/planning/README.md — it is authoritative if it disagrees with this line.
...
```
For each:
1. **Assign a milestone** per `docs/planning/README.md` triage criteria. **Precedence first:** `priority:critical` → the active release track (`v0.6.1`, the open patch line for shipped v0.6.0), regardless of theme; an isolated/minor `field-report` fix is biased toward `v0.6.1` the same way (not automatic — route by theme if it's actually a larger design gap). Otherwise by theme: `v0.6.1`=bug/patch fix to shipped v0.6.0 code (node/astro packs, JS validator/gate parity, pack `requires` resolution — epic #1029); `v0.7.0`=quality/non-blocking or worker/overseer finalization or other governance/security gaps in the active line; `v0.7.4`=testability (coverage/mutation scope, test-exemption accounting, shell→Python migration of decision logic, or a CI gate enforcing them); `v0.8.0`=agility; `Backlog`=no fit or needs human design decision. `v0.5.1` is drained and `v0.6.0` is closed to new triage — never select either. **`docs/planning/README.md` is the source of truth** — read it and prefer it over this list, which is a summary and may lag.
...
- `NEW WORK: BLOCKED`, reason cites `needs-fix-bounce` (a PR was bounced by the overseer — converted to draft and labeled `needs-ai`) → this is NOT "nothing to fix". Follow `.claude/agents/worker.md` §"Re-entry after a bounce (autonomous)": read the bounce comment's `### Specific failures`, fix each gap, then open a NEW PR referencing the bounced one (do not just push a commit to the draft PR — it stays excluded from the overseer's actionable-PR list while in draft), then STOP this iteration.
- `NEW WORK: BLOCKED`, reason cites `needs-fix` (a PR has `CHANGES_REQUESTED`, is conflicting, or has a human-applied `needs-ai` label without `needs-human` also present — the same handoff signal used elsewhere in this pipeline, applied even via a `COMMENTED` review rather than a formal `CHANGES_REQUESTED` one, #1522) → read that PR's reviews AND comments, fix the listed gaps, push a new commit, then — if the PR carries the `needs-ai` label — clear it (`bash bootstrap/edit_issue.sh --number <n> --remove-label needs-ai --app worker`), mirroring the `/approve`/`/decline` swap pattern for issues: the label is the "worker, act" signal, and leaving it set after acting re-triggers `needs-fix` routing on an already-fixed PR every later cycle (#1526). Then STOP this iteration.
...
2. CHANGES_REQUESTED → fix, push, clear `needs-ai` if present (`bash bootstrap/edit_issue.sh --number <n> --remove-label needs-ai --app worker`), STOP.
