...
- `NEW WORK: BLOCKED`, reason cites `needs-fix` (a PR has `CHANGES_REQUESTED`, is conflicting, or has a human-applied `needs-ai` label — the same handoff signal used elsewhere in this pipeline, applied even via a `COMMENTED` review rather than a formal `CHANGES_REQUESTED` one, #1522) → read that PR's reviews AND comments, fix the listed gaps, push a new commit, then STOP this iteration.
