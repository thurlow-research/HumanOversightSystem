...
TRIAGE RULE (for new issues): v0.5.1=patch release (bugs in shipped v0.5.0 code — fail-open, governance gap, regression); v0.6.0=Astro/JS stack support (node+astro packs, JS validator/gate parity — epic #1029); v0.7.0=quality/non-blocking (new capabilities, measurement); v0.8.0=agility. See docs/planning/README.md.
...
```
For each:
1. **Assign a milestone** per `docs/planning/README.md` triage criteria: `v0.5.1`=bug/governance gap in shipped code; `v0.6.0`=Astro/JS stack support; `v0.7.0`=quality/non-blocking; `v0.8.0`=agility; `Backlog`=no fit or needs human design decision.
