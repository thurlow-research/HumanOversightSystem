...
# `findings_key` is "findings" for agy, "attacks" for codex (matches each
# reviewer's own schema above). Uses python3 for the stderr tail so an
# embedded quote/backslash in vendor output can't break the JSON (the class
# of defect this ADR exists to remove).
_doc_validation_failure_json() {
...
Return JSON only:
{
  ...
  \"summary\": \"one paragraph overall assessment\"
    ...
    if vendor_invoke agy "$AI_REVIEW_TIMEOUT" "$prompt_file" "$stdout_file"; then
        ...
        result=$(_doc_validation_failure_json agy findings)
    fi
    echo "$result"
}
...
Return JSON:
{
  ...
  \"summary\": \"one paragraph\"
    ...
    if vendor_invoke codex "$AI_REVIEW_TIMEOUT" "$prompt_file" "$stdout_file"; then
        ...
        result=$(_doc_validation_failure_json codex attacks)
    fi
    echo "$result"
}
