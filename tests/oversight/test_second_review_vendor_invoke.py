...
def _run(tmp_path: Path, fake_bin: Path, *extra_args: str, path_override: str | None = None,
          score: str = "0.5") -> subprocess.CompletedProcess:
    env = dict(os.environ)
    env["PATH"] = path_override if path_override is not None else f"{fake_bin}:{env['PATH']}"
    return subprocess.run(
        ["bash", str(_SCRIPT), "--files", "target.py",
         "--step", "3", "--tier", "MEDIUM", "--score", score, *extra_args],
...
def _run_lib_snippet(tmp_path: Path, body: str, pre_source: str = "") -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        ["bash", str(snippet)], capture_output=True, text=True, timeout=30, env=env,
        cwd=str(tmp_path),
    )
...
def test_oversized_prompt_reaches_the_reviewer_on_stdin(tmp_path):
    ...
    fake_bin.mkdir()
    _write_exec(fake_bin / "agy", """#!/bin/sh
n=$(wc -c)
if [ "$n" -gt 131072 ]; then
...
def test_argv_still_carries_no_prompt_content(tmp_path):
    ...
    _write_exec(fake_bin / "agy", f"""#!/bin/sh
maxlen=0
for a in "$@"; do
    l=${{#a}}
...
def test_trap_chain_survives_existing_handler_with_single_quote(tmp_path):
    ...
    tmpdir = next(line.split("=", 1)[1] for line in r.stdout.splitlines() if line.startswith("TMPDIR="))
...
def test_trap_chain_survives_existing_handler_with_double_quote_and_semicolon(tmp_path):
    ...
    *shell* way to embed a single quote inside a single-quoted word, and using
    ...
    assert r.returncode == 0, f"stdout={r.stdout}\nstderr={r.stderr}"
    assert "existing" in r.stdout and "handler; ran" in r.stdout, f"stdout={r.stdout}\nstderr={r.stderr}"
    tmpdir = next(line.split("=", 1)[1] for line in r.stdout.splitlines() if line.startswith("TMPDIR="))
    assert not Path(tmpdir).exists(), f"vendor_invoke temp dir was not cleaned up: {tmpdir}"
