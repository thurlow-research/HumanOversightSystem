...
**Date:** 2026-09-23
**Status:** CLEARED FOR THE CODER, subject to the eleven binding conditions ARCH-1 … ARCH-11 in `docs/v0.7.0/ADR-1657-overseer-review-objects.md` §3 (see §14; §12 records the items that were routed to the architect and §13 the self-flag)
...
| HUMAN_REQUIRED, DIRTY, bounce | `**Executive summary:**` paragraph + the canonical `**Decision: HUMAN_REQUIRED**` header (**unconditionally** — see below) + one pointer line: `Full escalation: see the unresolved review thread on this PR.` | The §8.2 five-element escalation stays in the resolvable review thread via `post_review_thread.sh`, **unchanged** |
...
**The header is unconditional (ARCH-6).** The canonical `**Decision: HUMAN_REQUIRED**` marker appears in the verdict review body on **every** HUMAN_REQUIRED disposition — including DIRTY and bounce, which are HUMAN_REQUIRED sub-reasons (§3.3) — with no exceptions and no "where applicable" escape. The #761/#1215 consumers key on that exact string, and a conditional producer is precisely how this class of defect is built. A test in the wrapper suite pins it.

**Consequence that must be implemented, not merely noted.** Once the `**Decision: HUMAN_REQUIRED**` marker lives in a review body, the **#1215 duplicate-verdict precheck** — the consumer whose producer this design moves — must read review bodies. See §6.4. That repairs one half of VP-10, which is live today and unrelated to whether this design ships; it is recorded as a startup-gap in §11. The other consumer, the **#761 `prior_overseer_decision`** derivation, is **deliberately left unwired** (ARCH-7): wiring it would revive a dead guard and add a human gate to the LOW/MEDIUM non-protected path, which is a product decision routed to **#1839**, not a repair.
...
**One artifact, one dedup authority (ARCH-5).** The `:407` replacement deliberately states that the verdict review is **not** governed by the §1215 precheck. Deduplication of the verdict review is owned solely by wrapper guard **G4** (§4.3: byte-identical body, same head SHA). Two dedup authorities with different predicates — *"the latest marker is mine"* vs. *"the body is byte-identical"* — would suppress a verdict whose content **changed**, which is suppression of new information. The precheck continues to govern the §8.2 escalation thread and the narrative comment, which is why the clause at `:408` step (5) stays.

### 6.4 Lines 331–340 and 376–390 — the prior-decision marker now lives in review bodies

Both the #761 `prior_overseer_decision` derivation (`:331-340`) and the #1215 duplicate-comment precheck (`:376-390`) currently scan **only** `GET /repos/{o}/{r}/issues/{n}/comments`. Since #1207 the `**Decision: HUMAN_REQUIRED**` marker has been written into a **review thread**, and under this design the canonical copy moves into the **verdict review body**. So neither scan matches anything today — VP-10, which is live.

**VP-10 is repaired on one side only, and the split is binding (ARCH-5 / ARCH-7).** The **#1215 precheck** is rewired here: this design moves the producer, and leaving its consumer reading a surface the producer no longer writes — in the same change that moves the producer — would commit the #1207 producer/consumer defect a third time knowingly. The **#761 `prior_overseer_decision` derivation is left exactly as it is**, reading `/issues/{n}/comments` alone and therefore still dead. Rewiring it would not repair it, it would **revive** it: `merge_authority.py`'s guard then demands a verified human approval on the *current* head SHA for any PR that recorded `HUMAN_REQUIRED` on an earlier cycle, which is a new human gate on the LOW/MEDIUM non-protected path (§9 claim 1) and a product-boundary question rather than a repair. Status quo is not a regression. Whether to revive it, and what would clear it, is routed to **#1839** (`needs-human`, ARCH-8).
...
`:376-390` (the precheck) is then edited to say *"scan the union defined in § Where your own prior decision is recorded"* in place of its current single `GET /issues/{n}/comments` instruction. **`:331-340` (`prior_overseer_decision`) is left unchanged** and continues to scan `GET /issues/{n}/comments` alone (ARCH-7); the not-wired paragraph in the subsection above is what makes that inconsistency auditable rather than accidental. The precheck's rename from "duplicate-comment" to "duplicate-verdict" follows through to `:366` and `:408`.
...
| **W12** | **The livelock guard.** Two cases, both asserted, because I2's predicate is *any* review state on the current head (ARCH-9) and the table and the test must state the same rule: (a) the reviewer has an `APPROVED` review with `commit_id == head.sha`; (b) the reviewer has a `COMMENTED` review with `commit_id == head.sha`. In each case the reviewer is absent from `requested_reviewers` → exit 0, `skip_reason == "already_reviewed_head"`, **no POST**. The docstring must state the reason behaviourally and must **not** cite `merge_authority.py` by line number (ARCH-11): `decide_merge_authority` returns HUMAN_REQUIRED on an outstanding request from `human_reviewer` before it checks for any human approval, so a re-request after approval flips an approved PR back to HUMAN_REQUIRED every cycle. |
...
1. **The LOW/MEDIUM, non-protected auto-merge path is byte-for-byte unchanged in behaviour.** No reviewer is requested (mechanically impossible — §4.4 step 3 refuses), no new gate is introduced, no new label is applied, the merge call is untouched. **This claim is load-bearing on ARCH-7, and is only true because of it:** the #761 `prior_overseer_decision` derivation stays wired to `/issues/{n}/comments` alone — dead, exactly as today (§6.4). Rewiring it to the surface the marker now occupies would revive it, and a revived guard requires a verified human approval on the *current* head SHA for any PR that recorded `HUMAN_REQUIRED` on an earlier cycle, which **is** a new human gate on this path and would make this claim false. That revival is out of scope and routed to **#1839** (ARCH-8). The only change on this path is *where the findings narrative is posted* (review body instead of issues comment), which is required by change 1 of the issue and changes no gate. Pinned by **W8**, **R12**, **D4**.
...
| VP-10 (the #1215 / #761 marker scans reading the wrong surface) | **Yes**, and it is a *live* defect independent of this design — the third instance of the same class. | **Split, per ARCH-5/ARCH-7 — repaired on one side only.** The **#1215 precheck** is repaired in §6.4 rather than deferred: deferring it would ship a design that moves the producer while leaving its consumer reading a surface nothing writes. The **#761 `prior_overseer_decision`** derivation is **deliberately not repaired** — it stays reading `/issues/{n}/comments` alone, i.e. dead. Rewiring it would *revive* it and add a human-approval requirement on the LOW/MEDIUM non-protected path (§9 claim 1), which is a product-boundary decision, not a repair. §6.4's new subsection records the non-wiring in the file itself, and revival is routed to **#1839** (`needs-human`, ARCH-8). |
...
- **A1 — Ownership overlap with ADR-1357 slice 4.** ADR-1357 assigns *"any mutation, any GitHub write, … any refusal path (exit 3)"* to slice 4, and **AD-7** specifies `overseer_merge.sh` performing `POST /pulls/{n}/reviews` (APPROVE) → merge as one atomic unit. **[Premise corrected per ARCH-1.]** This item was routed on the premise that slice 4 is *"blocked on ESC-1, a human escalation with no recorded answer."* **That premise is false.** ESC-1 was **resolved by the human on 2026-09-11** (#1357's comment thread; already recorded at `docs/v0.7.0/TECHNICAL-DESIGN-1357-merge-authority-primitives.md:48`), and ADR-1357 §4's slice-4 row saying "Blocked on ESC-1" is stale text, not a live block — corrected in `ADR-1357-AMENDMENT-1` §2. Slice 4's **live** gates are **ESC-4** (four of `decide_merge_authority`'s inputs have no cross-clone transport) and **ESC-5** (whether the SPEC-303b CODEOWNERS gate gets a clearing path); both are open, and both are **disjoint from this surface** — they concern `decide_merge_authority`'s fresh recompute and the merge row of the matrix, and `pr_review.sh` performs no recompute, consumes none of the un-transported inputs, and merges nothing. That #1657 is `priority:critical` while every protected-surface PR currently merges by admin override is a **supporting** reason to land now, not the reason. **My design call:** ship `pr_review.sh` now as a single-purpose write primitive, adopting AD-2's envelope and AD-5's exit-3 refusal semantics verbatim, and record a **forward constraint on slice 4**: `overseer_merge.sh` and `overseer_escalate.sh` must call this primitive (or `pr_review_cli`'s functions in-process) for their approve/comment legs rather than re-implementing the POST — one invocation site, D41. Because AD-7's atomicity is about what the *agent* issues as one command, an internal call preserves it exactly. **What I needed from the architect:** confirmation that this is an authorised early landing of a slice-4-adjacent surface rather than a slice violation, and that the forward constraint is recorded against slice 4. **Ruled (ADR-1657 AD-1): proceed — authorised early landing.** ESC-1's own resolution makes the overseer *"always a required approver, at every tier"*, which is unimplementable until this primitive exists, so #1657 is a prerequisite of that ruling rather than a competitor to slice 4. FC-1/FC-2 are recorded in `docs/v0.7.0/ADR-1357-AMENDMENT-1-pr-review-primitive.md`.
...
1. **A1 — ADR-1357 slice-4 ownership.** Is landing `bootstrap/pr_review.sh` now an authorised early landing of a mutation surface that slice 4 nominally owns? (Architect first; human if the architect declines to bind.) **Answered — ADR-1657 AD-1: yes.** As originally written this item added *"given slice 4 is blocked on the unanswered ESC-1"*; per ARCH-1 that premise is false — ESC-1 was resolved on 2026-09-11, and slice 4's live gates (ESC-4/ESC-5) are disjoint from this surface (§12 A1).
