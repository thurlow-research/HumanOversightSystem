...
**Clarification, 2026-09-14 (architect, on TD-O2 from slice 1's technical design — a timing clarification, NOT a waiver).** The read-only clause above is binding but was written without slice phasing, which was a gap in this ADR rather than in the design that surfaced it. It is now dated: the read-only wrappers' "records that it ran" event lands **with the audit-write seam in slice 4, and no later than the slice in which an agent is first instructed to invoke the primitives (slice 6) — in the same PR as that prose or before it, never after.** Three reasons, in order of weight: (i) the clause's purpose is to make an *agent's claim to have run a control* verifiable, and until prose names the primitives no such claim exists — an event written in slice 1 records nothing anyone will ever check; (ii) AD-10's halt-on-failure write order is designed in slice 4, so a slice-1 producer would be a second, undesigned audit-write path, which is the duplicate-authority class AD-9 forbids; (iii) `bounce_count` linearly scans the entire committed trail (`merge_authority.py:1074-1092`) until #1538 CR-1's filtered reads land, so a per-primitive event in slice 1 would degrade the very function `bounce-count` wraps. **Binding on slice 4 and slice 6:** the unit of evidence is one event per invocation of an *agent-facing surface*, not one per L1 call — `overseer_decide` emits a single event carrying `checks[]`, and only a directly-invoked primitive emits its own. Slice 6's design must either implement this clause or record an explicit architect waiver; it may not simply omit it. **Affected sign-offs:** none. No design or code has been approved against AD-10 — slice 1 is unbuilt and its design is unapproved — so every prior sign-off stands and nothing is orphaned.
