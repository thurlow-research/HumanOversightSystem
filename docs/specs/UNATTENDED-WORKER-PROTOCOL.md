...
- **R5.3.3 — Benefit-≫-risk gate (computable).** The loop acts autonomously on a change **only when its expected benefit substantially outweighs the risk of the change**. The gate is computed as a coarse matrix function — unambiguous enough to write a test case against:
  ...
  | **P2 / P3** (medium/low severity) | **ACT** autonomously only if blast-radius is within per-run caps (§11.2) | **ESCALATE** to human |
  ...
  **Hard overrides (always force ESCALATE/HUMAN regardless of the matrix above):**
  ...
  "Blast-radius within per-run caps" for the P2/P3 row: ≤ the per-run caps in §11.2 (5 PRs / 10 issues / 25 files). "Security-relevance" and "protected-surface" are defined in R9.1.2 and R9.1.3 respectively. This matrix is the authoritative definition for test purposes.
...
- **R5.4.2 — Label-actor verification (reusing the §4.1.4/RT-F11 pattern).** GitHub does not restrict which account may apply a label, so the worker MUST verify that the `hos-autowork-authorized` label was applied by a **CODEOWNER** before honoring the authorization. The verification procedure reuses the §4.1.4 label-actor-verification pattern: read the label event actor from the GitHub API (`issues/{id}/events`, event type `labeled`, find the `hos-autowork-authorized` event), then check that actor against the CODEOWNERS file (the same CODEOWNERS used for branch-protection enforcement). A label applied by any actor who is NOT in CODEOWNERS does NOT authorize autonomous handling and is **ignored and logged** as a non-codeowner authorization attempt. The worker MUST NOT cache a "label is present" result — it must re-verify the label actor each time it evaluates whether the authorization is valid, in case the label was removed and re-applied by a different actor between probe cycles.
...
- **R7.5.5 — Hang timeout: a holder older than `max_task_runtime` is HUNG.** Read the start timestamp from `/tmp/hos-worker.lock/meta`. If `now − start_timestamp > max_task_runtime` (R11.4, default 4h), the holder is treated as HUNG regardless of whether the PID is still alive. A HUNG holder: (1) is reclaimed (remove `/tmp/hos-worker.lock`); (2) triggers the dead-man's-switch (R11.5) to page a human — a hung orchestrator holding the machine-global lock forever would wedge all future runs on this machine, which is itself an incident requiring human attention. Do not silently reclaim a HUNG lock without paging.
...
- **R7.5.7 — Layering: machine lock is PRIMARY; activation file + GitHub claim are COMPLEMENTARY.**
  ...
  - **Tie to O8 / the execution model (§3):** the cron-fired orchestrator is the process that holds the machine-local lock for the duration of its probe/dispatch cycle; per-task workers run *under* it, within its hold. The lock is not re-acquired per task — it is held by the orchestrator from probe start through dispatch completion, then released.
...
- **R8.2b** — The token estimate (R8.1) and blast-radius summary (§11.2) are part of element 1's risk picture for spend/merge escalations.
...
- **O17** — *(new — R7.5.2)* **Machine-local lock path (`/tmp/hos-worker.lock`) — confirm or adjust.** `/tmp/hos-worker.lock` is the specified canonical path. Confirm this does not conflict with any existing HOS or project tooling that uses `/tmp/hos-*`. If the running user does not have write access to `/tmp` in some deployment contexts (e.g. hardened containers), a fallback path (e.g. `~/.hos/worker.lock`) may be needed — the fallback must still be machine-global (not per-repo) and must document the change so all per-repo cronjobs agree on the path.
- **O18** — *(new — R7.5.4)* **Orchestrator script path for PID verification.** R7.5.4 requires `ps -p <pid> -o command=` to match the orchestrator script path. The exact match string or pattern to use (e.g. full path, basename, or a `grep` pattern against the command) must be defined during implementation so the alive-and-orchestrator check is not accidentally too broad (matching unrelated scripts) or too narrow (failing on path variations across machines).
- **O19** — *(new — R5.4.2)* **CODEOWNERS lookup mechanism for label-actor verification.** R5.4.2 requires checking the label-event actor against CODEOWNERS. The exact lookup mechanism needs implementation-phase confirmation: parse the repo's `.github/CODEOWNERS` file (or GitHub's CODEOWNERS API endpoint if one exists), determine the set of codeowners for the issue's files-at-triage-time, and check membership. Edge cases: wildcard CODEOWNERS patterns, team-based entries, and CODEOWNERS files that don't cover the root — the implementation must handle these without false-positives (treating a non-codeowner as a codeowner) or excessive API cost.
- **O20** — *(new — R5.4.1)* **`hos-autowork-authorized` label name — confirm.** The label name `hos-autowork-authorized` is specified in R5.4.1. Confirm this name fits the `hos-*` naming convention (R6.2) and does not conflict with any existing label. Add to the canonical label set (R6.2) and to T2 onboarding provisioning.
