...
|---|---|---|---|
| `needs-ai` | `.github/workflows/label-swap.yml` (`/approve`, `/handoff`); `merge_authority.py` (`labels_to_add` on the oversight-verdict-not-PROCEED path, `:599`; `record_pr_bounce()` default label, `:1068`); `self_review_source.py:229` (`file_finding_as_issue()`, alongside `hos-coordination`, at issue-creation time when filing a self-review finding). Removed on PRs by the worker itself after pushing a fix under the `needs-fix` path (`.claude/agents/worker.md` §"Step 1 — Check open PRs", `bootstrap/worker-cron-prompt.md` Step 1, both via `bootstrap/edit_issue.sh --remove-label needs-ai`), added `#1526` — mirrors `/approve`/`/decline`'s swap pattern for issues; nothing removed it on PRs before this. | **On issues:** work-selection filter — `probe.py:320`, `bin/hos-cron:1123` query by `labels=needs-ai`; `next_candidates.jq` excludes any issue also carrying `needs-human`. **On PRs:** `bin/hos-cron:1085` reads `draft == true` AND `needs-ai` present to detect an overseer bounce and route the worker to `needs-fix-bounce` (`#1350`, shipped `c4f5e905`); `bin/hos-cron:~1094` (added `#1522`) additionally treats `needs-ai` present on a still-open (non-draft) PR as `needs-fix`, so a human handing a PR back via a hand-applied label plus a `COMMENTED` review (not a formal `CHANGES_REQUESTED` one) is no longer read as "nothing to fix" — **except** when `needs-human` is also present on the same PR (`#1526`): `merge_authority.py`'s `labels_to_add` write path cannot distinguish a human-applied `needs-ai` from its own bot-applied one, so that combination falls through to the review-state checks (`CHANGES_REQUESTED`/`APPROVED` count) instead of forcing `needs-fix`. Otherwise not consulted by `merge_authority.py`'s own PR-label gate (`_HUMAN_GATE_LABELS`, below), which does not include it. | The same label name is used for two different directions on a PR (worker→overseer handoff vs. overseer→worker bounce), disambiguated by draft state plus, since `#1350`, by the label itself in the bounce-detection check. Before `#1526`, nothing ever cleared a PR-side `needs-ai` after the worker acted on it, so every subsequent cycle re-routed an already-fixed PR to `needs-fix` indefinitely (a livelock) — this is a narrow stopgap on the current mechanism, not a substitute for the `needs-worker`/`needs-overseer` rename below, which has no "never cleared" failure mode by construction. See `DECISIONS.md` 2026-09-06 entry (#1349) for that planned rename and why an earlier draft ruling to *not* rename was reversed. |
