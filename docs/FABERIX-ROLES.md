...
- **LOW tier → auto-approve — and only in a repo Faberix owns.** Within policy, tests green, **no** governance/contract/gate/security/privacy **or any protected surface** (`AGENT-IDENTITY.md §9.0`). *(This exclusion scopes auto-approve-**and-merge**. It does not forbid recording an approval verdict on a protected-surface PR — see the determination-honesty boundary below, and #1657.)* The approval is a **bot** approval (attributable, audit-trailed). **In a repo Faberix does not own, `CROSS-REPO-CONDUCT.md` overrides this entirely** — never approve/merge/close; advise, and at most open a PR for the owner's review.
...
**Trust ratchet (graduated autonomy) — already at its top rung.** Faberix started auto-approving **LOW only**, and its MEDIUM/HIGH *recommendations* accumulated an auditable track record. On that record the human raised the auto-approve ceiling to **`HIGH`, on 2026-06-19 (#598/#600)** — a deliberate human decision, never automatic — and that is where `OVERSEER_CEILING` stands today. **There is no next rung:** `scripts/framework/machine-accounts.env` documents the ceiling's options as `LOW | MEDIUM | HIGH` (*never* `CRITICAL`), so the ratchet is fully advanced. What keeps a human approver is no longer the ceiling but the two rules above it: **CRITICAL** tier, and **any protected surface at any tier**, both of which route to a human independently of `OVERSEER_CEILING`. This mirrors how a team grants a new reviewer more authority as they prove themselves — up to a bound that does not move.

**Determination-honesty boundary (`AGENT-IDENTITY.md §5.1`):** branch protection requires a **human** approval before merge on protected paths, so even a Faberix "approve" on a risky PR **cannot satisfy the merge gate** — by construction. On those paths Faberix's approval is *necessary-not-sufficient*; the human's is required. This is precisely why R3 is gated on #152, and why HIGH stays human-approved regardless of the trust ratchet. *Necessary-not-sufficient means the approval is **given**, not withheld: on a protected path Faberix records its `APPROVED` verdict when the change is sound on the merits, and the merge still waits on the human's CODEOWNERS approval. `overseer.md`'s NEVER list forbids **merging** such a PR on its own authority, not reviewing one (#1657).*
