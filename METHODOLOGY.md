...
**The research subject is the oversight layer** — how signals become *scaled human oversight*. The signal generators are inputs. Several of them — complexity, N+1, function metrics, portability, diff size — are ordinary **software-quality** checks, so running them also makes the product better; that improvement is a **byproduct, a benefit — not the research claim**. The contribution is the *routing of human attention over aggregated signals*, not the individual metrics, which are cheap, replaceable proxies. The signal set is **extensible**: a project can register its own signal generators (see #80) and the oversight layer consumes them unchanged.

### How the composite score is computed

Each build step runs a set of deterministic validator scripts (`scripts/oversight/run_validators.sh`). **Thirteen validator scripts each produce one scored dimension** tabulated below. Every dimension produces a score in [0.0, 1.0] and a weight. The composite is a weighted average across all validators that ran without error — validators that fail to execute are excluded rather than zeroing the score, so a misconfigured tool degrades coverage gracefully rather than suppressing the signal entirely.

**Testability as a code-quality principle (#314, #1241).** The question a reviewer or validator asks is never *"does this have tests"* but *"is it structured so it CAN have unit tests."* Concretely: shell scripts are launchers, not logic containers — a `.sh` file that evaluates a decision (a conditional on program/API output, a retry loop, a data transformation, anything with more than one outcome) sits in front of the tested code in the worst available place, because coverage and mutation tooling do not reach shell. Logic belongs in a language inside the test regime; shell's job is fixed flag parsing, literal paths, and exec into that tested code. `code-reviewer.md` carries this as an explicit review criterion, and the `shell_logic` validator dimension below gives it a deterministic signal.
...
**The validator dimensions and their weights.** The **Signal type** column tags *what each generator detects*. The `quality` signals (complexity, N+1, function metrics, portability, diff size, shell logic) are ordinary software-quality measures — running them improves the product, which is a benefit, but they are signal *sources*, not the research subject. The `correctness / security / provenance / fidelity / hallucination` signals are the AI-code-specific risk axes; `oversight-loop` is a signal the oversight layer feeds back to itself (escape-rate calibration). All thirteen are **inputs** the oversight layer aggregates — the aggregation-and-routing is the contribution, not any individual metric.
...
| **Complexity** | 0.08 | quality | McCabe cyclomatic complexity (independent execution paths — testability), scored by `complexity_metrics.py` via radon. Cognitive complexity (Campbell 2018) — a readability signal — was intended as a second, independent dimension but is not currently computed: radon's `cc` yields cyclomatic only. |
...
| **Diff size** | 0.00 | quality | Change-size proxy for review difficulty and blast radius (lines/files changed). Inert in the composite (weight 0.0) — it acts as a discrete **tier floor** rather than a weighted term (#377). |
| **Shell logic** | 0.05 | quality | Decision-construct density (`if`/`elif`/`case`/`while`/`until`) in changed `.sh` files, excluding the sanctioned fixed-flag-parsing loop and `bootstrap/hos_bootstrap.sh` (machine bootstrap must be shell — it installs Python). Signal for #314's shell-vs-Python policy: shell carrying decision logic sits untested in front of tested code (`shell_logic_check.py`). |

> **Quality signals are a benefit and a signal source — not the research subject.** Six of the thirteen dimensions (`quality`) are conventional software-quality checks. They earn their place by *feeding the oversight layer* a cheap proxy for review difficulty and defect-proneness, and as a side effect they make the generated code better. The research claim does **not** rest on them: swap cyclomatic complexity for any other quality proxy and the oversight contribution — aggregate, stratify, route human attention, gate, ratchet, audit — is unchanged. This is why the signal set is **extensible** (#80): a project registers its own quality (or domain) signals and the oversight layer consumes them without modification.
...
| **`scripts/setup_oversight.sh`** | *Legacy* project installer (AGENTS.md, CODEOWNERS, PR template, `.claude/settings.json`, capture/audit scripts, `prompts/`, branch protection). **Removed** — superseded by `bootstrap/hos_install.sh` (release-pinned install); reconciliation tracked in #87. | ❌ removed |
