...
| Agent | Model | Role |
|---|---|---|
| **risk-assessor** | Sonnet | Runs after coding, before reviewers. Applies deterministic floor rules, runs validator scripts, scores the code across 9 dimensions, validates the coder's self-declared risk tier (can only raise, never lower), and produces a ranked inspection brief directing reviewer attention. |
| **dep-mapper** | Sonnet | Subagent of risk-assessor. Builds the Django dependency graph for changed files: fan-in (who imports this), signal connections, template inheritance, middleware stacking. Produces blast-radius report. |
| **risk-historian** | Haiku | Subagent of risk-assessor. Queries GitHub issues and git log for historical bug density and churn on changed files. Starts empty on new projects; accumulates value over time. |
| **spec-red-team** | Sonnet | Runs before coding begins on each build step. Uses agy adversarially to find gaming vectors, contradictions, and implicit assumptions in the spec before implementation locks them in. Creates `spec-gap` issues. |
| **oversight-evaluator** | Sonnet | Runs after all internal reviewers have approved and system tests pass. Phase 1: checks compliance — did all required sign-offs happen? Phase 2: evaluates quality — convergence failures, critical findings resolved, confidence gaps. Produces: PROCEED / CONDITIONAL\_PROCEED / ESCALATE. |
| **oversight-orchestrator** | Sonnet | Acts on the evaluator's recommendation. PROCEED → opens PR + writes handoff document + prints panel command. CONDITIONAL → same, with "Human Review Required Before Merge" section. ESCALATE → surfaces specific questions to human, PR does not open. |
